...
// Form handler (POST to Google Apps Script)
async function handleFormSubmit(e) {
  ...
  try {
    ...
    btn.classList.replace("bg-primary-600", "bg-emerald-600");
    form.reset();
  } catch (error) {
    console.error("Error!", error.message);
    btn.innerHTML =
      '<i class="fas fa-exclamation-triangle"></i> Failed to Send';
    btn.classList.replace("bg-primary-600", "bg-rose-500");
  } finally {
    setTimeout(() => {
      btn.innerHTML = originalHTML;
      btn.classList.remove("bg-emerald-600", "bg-rose-500");
